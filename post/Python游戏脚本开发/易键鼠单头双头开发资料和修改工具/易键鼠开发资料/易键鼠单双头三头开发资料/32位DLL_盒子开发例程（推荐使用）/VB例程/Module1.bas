...
'扫描硬件列表，打开扫描到的第一个单头或者双头盒子
 Declare Function M_ScanAndOpen Lib "msdk.dll" () As Long
'关闭端口；在程序退出前再关闭端口
...
'读取VID/PID; IdIndex=1/2/3/4 分别对应 主控端Vid, 主控端Pid, 被控端Vid, 被控端Pid
 Declare Function M_GetVidPid Lib "msdk.dll" (ByVal m_hdl As Long, ByVal IdIndex As Long) As Long
'延时指定时间
...
'设置DLL内部参数
 ...
 Declare Function M_GetAbsCfg Lib "msdk.dll" (ByVal m_hdl As Long) As Long
 '设置绝对移动功能: e1d0=1 打开; =0;关闭
 Declare Function M_SetAbsCfg Lib "msdk.dll" (ByVal m_hdl As Long, ByVal e1d0 As Long) As Long
 '是否支持修改产品名称, 返回值1=支持，0=不支持；-1，失败
 Declare Function M_ChkSupportProdStrMdy Lib "msdk.dll" (ByVal m_hdl As Long) As Long
 '读取产品名称
 Declare Function M_GetProdStr Lib "msdk.dll" (ByVal m_hdl As Long, ByVal m0s1 As Long, ByRef LenOfStr As Long, ByRef ucp_Str As String) As Long
 '设置产品名称
 Declare Function M_SetProdStr Lib "msdk.dll" (ByVal m_hdl As Long, ByVal m0s1 As Long, ByVal LenOfStr As Long, ByVal ucp_Str As String) As Long
 '是否支持加密功能, 返回值1=支持，0=不支持；-1，失败
 Declare Function M_ChkSupportEnc Lib "msdk.dll" (ByVal m_hdl As Long) As Long
 '检测密码状态; 返回值: <0 错误；255=无密码; 0~16:密码剩余有效次数
 Declare Function M_ChkPwStat Lib "msdk.dll" (ByVal m_hdl As Long) As Long
 '设置新密码
 Declare Function M_SetPw Lib "msdk.dll" (ByVal m_hdl As Long, ByVal PwLen As Long, ByVal PwMaxCnt As Long, ByRef ucp_pw As Byte) As Long
 '验证密码
 Declare Function M_VerifyPw Lib "msdk.dll" (ByVal m_hdl As Long, ByVal PwLen As Long, ByRef ucp_pw As Byte) As Long
 '写入数据
 Declare Function M_WrEncData Lib "msdk.dll" (ByVal m_hdl As Long, ByVal DataLen As Long, ByRef ucp_data As Byte) As Long
 '读取数据
 Declare Function M_RdEncData Lib "msdk.dll" (ByVal m_hdl As Long, ByVal DataLen As Long, ByRef ucp_data As Byte) As Long
