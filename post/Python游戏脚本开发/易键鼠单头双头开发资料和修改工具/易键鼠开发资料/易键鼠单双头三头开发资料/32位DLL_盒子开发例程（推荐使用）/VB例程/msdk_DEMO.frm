...
Begin VB.Form Form1 
   ...
   Begin VB.CommandButton EnDecTest 
      Caption         =   "加密功能测试"
      BeginProperty Font 
         Name            =   "宋体"
         Size            =   10.5
      ...
      Width           =   1935
   End
   Begin VB.CommandButton ScanOpenPort 
      Caption         =   "扫描本机设备列表并打开盒子端口"
      BeginProperty Font 
         Name            =   "宋体"
         Size            =   10.5
         Charset         =   134
      ...
      Width           =   2175
   End
   Begin VB.CommandButton ReadVidPid 
      Caption         =   "读取VID/PID"
      Height          =   375
      Left            =   5520
      TabIndex        =   53
...
Private Sub OpenPort_VidPid_Click()
    ...
    End If
End Sub
Private Sub ScanOpenPort_Click()
    If M_Handle > 0 Then
        MsgBox "端口已经打开，请不要重复打开端口。请关闭端口后才能再次打开端口!"
        Exit Sub
    End If
    M_Handle = M_ScanAndOpen()
    If M_Handle < 0 Then
        MsgBox "打开端口失败!"
    Else
        OpenPort.Enabled = False
...
Private Sub GetSn_Click()
    ...
    If RetCode = 0 Then
        ...
        While n < dwp_LenResponse
            uc_Response = uc_Response & Hex(ucp_Response(n)) & " "
            n = n + 1
        Wend
        MsgBox uc_Response
    Else
...
Private Sub EnDecTest_Click()
    ...
    RetCode = M_ChkSupportEnc(M_Handle)
    If RetCode <> 1 Then
        MsgBox "不支持加密功能"
        Exit Sub
    End If
    
    RetCode = M_ChkPwStat(M_Handle)
    If RetCode < 0 Then
        MsgBox "查询密码状态失败"
    ElseIf RetCode = 255 Then
        MsgBox "还未设置密码"
    Else
        MsgBox "密码剩余有效次数" & RetCode
    End If
    
    AsciiText = "12345678"
    ucp_Cmd = StrConv(AsciiText, vbFromUnicode)
    CmdLen = Len(AsciiText)
    RetCode = M_SetPw(M_Handle, CmdLen, 12, ucp_Cmd(0))
    If RetCode <> 0 Then
        MsgBox "设置密码失败。错误码：" & RetCode
    End If
    RetCode = M_VerifyPw(M_Handle, 8, ucp_Cmd(0))
    If RetCode <> 0 Then
        MsgBox "验证密码失败。错误码：" & RetCode
        Exit Sub
    End If
    
    AsciiText = "1234567890"
    ucp_Cmd = StrConv(AsciiText, vbFromUnicode)
    CmdLen = Len(AsciiText)
    RetCode = M_WrEncData(M_Handle, CmdLen, ucp_Cmd(0))
    If RetCode <> 0 Then
        MsgBox "写加密数据失败。错误码：" & RetCode
        Exit Sub
    End If
    
    RetCode = M_RdEncData(M_Handle, 10, ucp_Response(0))
    If RetCode < 0 Then
        MsgBox "读加密数据失败。错误码：" & RetCode
    Else
        uc_Response = "所读取到数据："
        n = 0
        While n < RetCode
...
Private Sub ReadVidPid_Click()
    ...
    If M_Handle < 0 Then
        ...
        Exit Sub
    End If
    mVid = M_GetVidPid(M_Handle, 1)
    mPid = M_GetVidPid(M_Handle, 2)
    sVid = M_GetVidPid(M_Handle, 3)
    sPid = M_GetVidPid(M_Handle, 4)
    MsgBox "十六进制：" & vbCrLf & "主控端Vid=0x" & Hex(mVid) & "  主控端Pid=0x" & Hex(mPid) & vbCrLf & "被控端Vid=0x" & Hex(sVid) & "  被控端Pid=0x" & Hex(sPid) & vbCrLf & vbCrLf & _
           "十进制：" & vbCrLf & "主控端Vid=" & mVid & "  主控端Pid=" & mPid & vbCrLf & "被控端Vid=" & sVid & "  被控端Pid=" & sPid
End Sub
Private Sub KeyPress_Click()
    ...
    RetCode = M_ReleaseAllMouse(M_Handle)
...
Private Sub KeyInputString_Click()
    ...
    'RetCode = M_KeyInputString(M_Handle, AsciiText, AsciiLen)
